...
// Heuristically identifies guest CPU fences in a PM4 command list. GPU-to-GPU label waits are
// removed from the candidate list so deferred readback work is committed only at likely CPU-visible
        ...
        while (!cmd.empty()) {
            ...
            case 3: {
                ...
                case PM4ItOpcode::EventWriteEop: {
                    ...
                    if (event_eop->int_sel.Value() != InterruptSelect::None) {
                ...
                case PM4ItOpcode::ReleaseMem: {
                    ...
                    if (release_mem->data_sel.Value() == DataSelect::Data32Low) {
                        fences.emplace_back(header, release_mem->Address<VAddr>(),
                                            release_mem->DataDWord());
                    } else if (release_mem->data_sel.Value() == DataSelect::Data64) {
                        fences.emplace_back(header, release_mem->Address<VAddr>(),
                ...
                case PM4ItOpcode::WriteData: {
                    ...
                    const u32 data_size = write_data->Size();
